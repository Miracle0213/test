...
const uint8_t init_cmd[] = {
    ...
    1, CMD_VCOMS, 0x35,     // VCOM setting 0.725v (default 0.75v for 0x20)
    1, CMD_LCMCTRL, 0X2C,   // LCMCTRL, Default value
    1, CMD_VDVVRHEN, 0x01,  // VDV and VRH command Enable, Default value
    1, CMD_VRHS, 0x13,      // VRH set, +-4.45v (default +-4.1v for 0x0B)
...
void LCD_FillPixels(uint16_t pixels, uint16_t color)
{
    ...
    if (config.dma_sz == mode_16bit)
    {
        color = (color << 8) | (color >> 8);  // Swap bytes for DMA 16bit mode
    }
    ...
    else
    {
        ...
        for (uint16_t t = 0; t < (pixels < DMA_min_Sz ? pixels : DMA_min_Sz); t++)
        {  // Fill the buffer with the color
            fill[t] = color;
        }
        while (pixels)
        {  // Send 64 pixel blocks
            uint16_t sz = (pixels < DMA_min_Sz ? pixels : DMA_min_Sz);
...
int8_t LCD_Fill(uint16_t xSta, uint16_t ySta, uint16_t xEnd, uint16_t yEnd, uint16_t color)
{
    ...
    {
        return GUI_RESULT_FAIL;
    }
    if (xSta >= LCD_WIDTH || ySta >= LCD_HEIGHT)
    {
        return GUI_RESULT_FAIL;
    }
    if (xEnd >= LCD_WIDTH)
    ...
    uint32_t pixels = (uint32_t)(xEnd - xSta + 1) * (uint32_t)(yEnd - ySta + 1);
    ...
    while (pixels > 0)
    {
        uint16_t chunk = (pixels > 0xFFFFU) ? 0xFFFFU : (uint16_t)pixels;
        LCD_FillPixels(chunk, color);
        pixels -= chunk;
    }
#ifdef USE_DMA
    setDMAMemMode(mem_increase, mode_8bit);
...
int8_t LCD_DrawLineByColor(uint16_t x0, uint16_t y0, uint16_t *color_buff, uint16_t length)
{
    ...
    LCD_SetAddressWindow(x0, y0, x1, y0);
    ...
    setSPI_Size(mode_16bit);  // Set SPI to 16 bit
#endif
    LCD_WriteData((uint8_t *)color_buff, length);  // Each pixel is 2 bytes

#ifdef USE_DMA
    setDMAMemMode(mem_increase, mode_8bit);  // Set SPI and DMA to 8 bit, enable memory increase
#else
    setSPI_Size(mode_8bit);  // Set SPI to 8 bit
#endif
...
// {
//     UG_FontSelect(font);
//     uint16_t color1 = (color & 0xF800) | ((color >> 5) & 0x07E0) | (color & 0x001F);
//     UG_SetForecolor(color1);
//     uint16_t bgcolor1 = (bgcolor & 0xF800) | ((bgcolor >> 5) & 0x07E0) | (bgcolor & 0x001F);
//     UG_SetBackcolor(bgcolor1);
...
void LCD_init(void)
{
    ...
    HAL_Delay(30);
    LCD_WriteCommand((uint8_t *)CMD_SLPOUT, 0);
    HAL_Delay(30);
    LCD_PIN(LCD_RST, SET);
    HAL_Delay(150);
#endif
