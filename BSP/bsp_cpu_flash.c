...
#define CPU_FLASH_END_ADDR (uint32_t)(0x08001FFF)
...
{
    return (Address - CPU_FLASH_BASE_ADDR) / FLASH_PAGE_SIZE;
}
...
uint8_t bsp_EraseCpuFlash(uint32_t _ulFlashAddr)
{
    ...
    __HAL_FLASH_CLEAR_FLAG(FLASH_FLAG_EOP | FLASH_FLAG_WRPERR | FLASH_FLAG_PROGERR | FLASH_FLAG_PGAERR | FLASH_FLAG_PGSERR);
...
*	功能说明: 写数据到CPU 内部Flash。 必须按8字节(双字)整数倍写。不支持跨扇区。扇区大小2KB.
*			  写之前需要擦除扇区. 长度不是8字节整数倍时，最后几个字节末尾补0写入.
*	形    参: _ulFlashAddr : Flash地址
*			 _ucpSrc : 数据缓冲区
...
uint8_t bsp_WriteCpuFlash(uint8_t *_ucpSrc, uint32_t _ulFlashAddr, uint32_t _ulSize)
{
    ...
    {
        bsp_EraseCpuFlash(_ulFlashAddr);
    }

    /* FLASH 解锁 */
    HAL_FLASH_Unlock();

    for (i = 0; i < _ulSize / 8; i++) /* 未处理非8字节整数倍个数情况 */
    {
        ...
        if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, _ulFlashAddr, FlashWord) == HAL_OK)
        {
            _ulFlashAddr = _ulFlashAddr + 8; /* increment for the next Flash word*/
        }
        else
        {
            goto err;
