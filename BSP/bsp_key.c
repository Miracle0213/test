...
static const X_GPIO_T s_gpio_list[HARD_KEY_NUM] = {
    { GPIOA, GPIO_PIN_4, 0 }, /* K1 */
    { GPIOA, GPIO_PIN_5, 0 }, /* K2 */
    { GPIOA, GPIO_PIN_6, 0 }, /* K3 */
    { GPIOA, GPIO_PIN_7, 0 }, /* K4 */
    { GPIOB, GPIO_PIN_0, 0 }, /* K5 */
...
static uint8_t IsKeyDownFunc(uint8_t _id)
{
    ...
    if (_id < HARD_KEY_NUM)
    {
        ...
        if (count == 1 && save == _id) { return 1; /* 只有1个键按下时才有效 */ }
...
static void bsp_InitKeyHard(void)
{
    ...
    gpio_init.Pull = GPIO_PULLUP; /* 上拉电阻不使能 */
...
void bsp_PutKey(uint8_t _KeyCode)
{
    ...
    if (++s_tKey.Write >= KEY_FIFO_SIZE) { s_tKey.Write = 0; }
    //    SEGGER_RTT_printf(0, "bsp_PutKey %d\r\n", _KeyCode);
}
...
uint8_t bsp_GetKey(void)
{
    ...
    //    SEGGER_RTT_printf(0, "bsp_GetKey %d\r\n", ret);
}
...
uint8_t bsp_GetKey2(void)
{
    ...
    else
    {
        ...
        if (++s_tKey.Read2 >= KEY_FIFO_SIZE) { s_tKey.Read2 = 0; }
        return ret;
    }
}
...
void bsp_KeyScan10ms(void)
{
    ...
    for (i = 0; i < KEY_COUNT; i++) { bsp_DetectKey(i); }
}
...
void bsp_KeyScan1ms(void)
{
    ...
    for (i = 0; i < KEY_COUNT; i++) { bsp_DetectFastIO(i); }
}
